...
//          n           m1          b1      v1      ev1     l1    m2        b2      v2      ev2    l2    template           prn
    ...
    new Elm(IFI0,       XX,         "-1",   "-1",   "-1",   -1,   XX,       "-1",   "-1",   "-1",   1,   "jrnm %s",         oneparam);
...
//          n           m1          b1      v1      ev1     l1    m2        b2      v2      ev2    l2    template           prn
    ...
    new Elm(TO,         R8,         "a",    "-1",   "-1",   -1,   R8,       "a",    "-1",   "-1",   1,    "ld %s,%s",       blank);
...
let segtype seg = match seg with | "reg" -> "reg" | "ram" -> "ram" | "eeprom" -> "eeprom" | "flash" -> "flash" | _ -> "unify"
...
let generateCode cmd = if currentseg <> "flash" then currentseg <- "flash"; "flash\tsegment\t'flash'\n\t" else "\t" + cmd+"\n"
